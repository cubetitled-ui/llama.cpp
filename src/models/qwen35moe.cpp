...
llama_model_qwen35moe::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    auto build_layer = [&](int il, int bloop = 0, int bloops = 1, bool is_alt = false) {
        ...
        if (hparams.is_recr(il)) {
            // Linear attention layer (gated delta net) - natively recurrent
            cur = build_layer_attn_linear(inp->get_recr(), cur, il);
        } else {
            // Full attention layer
            cur = build_layer_attn(inp->get_attn(), cur, inp_pos, sections, il, get_store_kv(bloop, bloops, is_alt));
        }
    ...
    if (block_loops > 1 && block_end >= block_start) {
        ...
        const float alpha = get_recurrent_block_alpha(0, block_loops, model.arch, model.hparams.n_embd);
        const float exit_alpha = get_recurrent_block_exit_alpha(model.arch, model.hparams.n_embd, block_loops);
