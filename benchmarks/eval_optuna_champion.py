...
OUTPUT_FILE = "/home/cune/llama.cpp/benchmarks/champion_momentum_benchmark_results.json"

CONFIG = {
    "id": "champion_momentum_t2",
    "name": "Optuna Champion (Mode 2 SNC-MD, L=14, g=0.0331, T=2)",
    "layer": 14,
    "gamma": 0.0331,
    "mode": 2,
    "t": 2
...
def eval_task(cat: str, task: dict) -> Tuple[bool, float, str]:
    if cat == "agentic_loop":
        ...
        out2, d2 = run_inference(t2_prompt, max_tokens=128)
        ...
        prompt = task["prompt"]
        out, dur = run_inference(prompt, max_tokens=256)
